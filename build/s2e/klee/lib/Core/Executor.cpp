...
Executor::Executor(const InterpreterOptions &opts,
  ...
  //addbyxqx201303
  m_kleeInfoFile = interpreterHandler->openOutputFile("klee-info.txt");
  m_kleeInfoFile->setf(std::ios_base::unitbuf);
  std::streambuf* m_kleelogStreamBuf = m_kleeInfoFile->rdbuf();
  
  m_kleeInfoFile->rdbuf(m_kleelogStreamBuf);
  m_kleeInfoFile->setf(std::ios_base::unitbuf);
...
Executor::toConstantSilent(ExecutionState &state,
                     ref<Expr> e) {
  //addbyxqx201302 
  //std::cerr << "test\n" ;  // print message on the monitor
  //klee_message("test %d\n", 1);  //print message in the debug.txt with a prefix "KLEE"
  if(e.isNull())
	  std::cerr << e << "---\n";
...
//****************************************************
// addbyxqx2013
void xgdb_printExpr(klee::Expr *e) {
	std::cerr << "expr: " << e << " -- ";
	if (e) {
		std::cerr << *e;
	} else {
...
//addbyxqx201303
void xgdb_printVector(std::vector< unsigned char > v) {
	std::cerr << "vector: size=" << v.size() << "\n";
	for(unsigned int i=0; i< v.size(); i++){
		//std::cerr << std::hex << v[i] << "  ";
		std::cerr << std::setw(2) << std::setfill('0') << (unsigned) v[i] << ' '
		<< (v[i] >= 0x20 ? (char) v[i] : ' ');
	}
	std::cerr << "\n";
}

void xgdb_printArray(const Array *a) {
	std::cerr << "ArrayName = " << a->name << "size = " << a->size << "\n";
	std::cerr << "ExprVector - constantValues.size() = " << a->constantValues.size() << "\n" ;
	for(unsigned int i=0; i< a->constantValues.size(); i++){
		xgdb_printExpr(a->constantValues[i].get());
		std::cerr << "\n";
	}
	std::cerr << "\n";
...
void xgdb_printVectorArray(std::vector<const Array* > v) {
	std::cerr << "vector: size=" << v.size() << "\n";
	for(unsigned int i=0; i< v.size(); i++){
		xgdb_printArray(v[i]);
	}
...
//addbyxqx201303  copyfrom mjj-klee
...
//addbyxqx201303  copyfrom mjj-klee
void Executor::xgdb_printState(ExecutionState &state, bool fullMO )
{
	//getKleeInfoStream() << "Executor::xgdb_printState info \n"; 
	
	//if (klee_message_stream) {
	//	*klee_message_stream << "klee_message_stream test\n" ;
	//}
	
	std::cerr << "State [" /*<< state.getID()*/ << "] : "<< static_cast<void *>(&state) << "\n";
	...
	ExprPPrinter::printConstraints(std::cerr, state.constraints);
	
	std::cerr << "+====================================================\n";
}

//addbyxqx201303  copyfrom mjj-klee
void Executor::xgdb_printState(std::ostream &ostrm, ExecutionState &state, bool fullMO )
{
	//getKleeInfoStream() << "Executor::xgdb_printState info \n"; 
	...
	ostrm << "State [" /*<< state.getID()*/ << "] : "<< static_cast<void *>(&state) << "\n";
	ostrm << "+-- Instruction at ";
	const InstructionInfo &ii = *state.pc->info;
	if(ii.file != "")
		ostrm << ii.file << ":" << ii.line << ":\n";
	else
		ostrm << "[No debug info]\n";
	...
	{
		const MemoryObject *mo = *it;
		ostrm << "| \t\t MO id = " << std::setw(6) << mo->id << "\t ";
		ostrm << "(" << (mo->address) << "," << mo->size << ")\n";
	}
	
	int nrLocals = sf.kf->numRegisters;
	ostrm << "|\t locals [0-" << nrLocals -1 << "]:\n";
	for(int i=0; i<nrLocals; i++) {
		Cell *c = sf.locals +i ;
		if(c->value.get() == NULL)
			continue;
		ostrm << "|\t\t" << i << ":";
		c->value.get()->print(ostrm);
		ostrm << "\n";
	}
	
	//print memory address mapping
	ostrm << "+-- Memory Mapping\n";
	if(fullMO) {
		MemoryMap &mm = state.addressSpace.objects;
		MemoryMap::iterator it = mm.begin();
		MemoryMap::iterator ie = mm.end();
	...
	//print Constraints
	ostrm << "+-- Constraints \n";
	ExprPPrinter::printConstraints(ostrm, state.constraints);
